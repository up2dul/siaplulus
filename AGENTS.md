...
export const createPostsApi = (client: Client) => ({
  ...
  getById: (id: string) => client.GET('/posts/{id}', { params: { path: { id } } }),
...
```bash
# Development
moon run web:dev              # Frontend at http://localhost:5173
...
The `pre-commit` hook runs Biome (JS/TS) and Ruff (Python) automatically on staged files.
The `commit-msg` hook validates the commit format.
